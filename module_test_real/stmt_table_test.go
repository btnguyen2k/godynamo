...
func Test_Query_ListTables(t *testing.T) {
	...
	for _, tbl := range tableList {
		...
		timeout, waitTime := 30*time.Second, 500*time.Millisecond
...
func Test_Exec_AlterTable(t *testing.T) {
	...
	timeout, waitTime := 30*time.Second, 500*time.Millisecond
	{
		_, err := db.Exec(fmt.Sprintf(`CREATE TABLE %s WITH PK=id:string WITH CLASS=standard`, tblName))
		if err != nil {
			t.Fatalf("%s failed: %s", testName+"/create_table", err)
		}
		ctx, ctxCancel := context.WithTimeout(context.Background(), timeout)
	...
	testData := []struct {
		...
		// Note: changing billing model from PROVISIONED to PAY_PER_REQUEST will take a very long time; thus not testing here
		// (change billing model from PAY_PER_REQUEST to PROVISIONED is much faster)
		{name: "change_wcu_rcu_provisioned", sql: fmt.Sprintf(`ALTER TABLE %s WITH wcu=3 WITH rcu=5`, tblName), affectedRows: 1,
			tableInfo: &tableInfo{tableName: tblName, class: "STANDARD", billingMode: "PROVISIONED", wcu: 3, rcu: 5, pkAttr: "id", pkType: "S"}},
		{name: "change_table_class", sql: fmt.Sprintf(`ALTER TABLE %s WITH CLASS=STANDARD_IA`, tblName), affectedRows: 1,
			tableInfo: &tableInfo{tableName: tblName, class: "STANDARD_INFREQUENT_ACCESS", billingMode: "PROVISIONED", wcu: 3, rcu: 5, pkAttr: "id", pkType: "S"}},
	}
